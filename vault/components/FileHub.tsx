...
function OutboundIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      <path d="M3 9l9-5 9 5v10H3V9z" strokeLinejoin="round" />
      <path d="M9 19V12h6v7" strokeLinejoin="round" />
...
export function FileHub() {
  ...
  async function processUpload(staged: StagedFile[]) {
    ...
    try {
      ...
      if (zipBlob.size > MAX_UPLOAD_BYTES) {
        throw new Error(`ファイルサイズ（${formatSize(zipBlob.size)}）が上限を超えています`);
      }
      ...
      setPhase('uploading');
      setProgress(0);
      setStatusLabel(`転送中: ${zipName}`);
      await uploadFn(parcel, (n) => setProgress(n));

      push(`${zipName} をアップロードしました`, 'success');
      await loadFiles();
    } catch (err) {
  ...
  const phaseLabel =
    phase === 'collecting' ? '読み込み中' :
    phase === 'packing' ? 'ZIP梱包中' :
    ...
    <div className="hub">
      <section className="hero illus-wrap anim anim--1">
        <div className="hero__text">
          <span className="pill anim-pill">自宅PC → 社内PC</span>
          <h1>PC間でファイルを送る</h1>
          <p>自宅からファイルをアップロードし、社内PCでダウンロード。選んで送るだけのファイル転送です。</p>
      ...
      <section className="ship-card ship-card--outbound anim anim--3">
        <div className="ship-card__banner">
          <span className="ship-card__banner-icon" aria-hidden="true">
            <OutboundIcon />
          </span>
          <div className="ship-card__head">
            <div>
              <h2>出荷倉庫（自宅PC）</h2>
              <p className="ship-card__sub">ファイル選択 → 自動ZIP梱包 → アップロード</p>
            </div>
        ...
        <div className="ship-card__body">
          ...
          >
            ...
            <p className="drop-port__title">
              {busy ? phaseLabel : 'ここにファイルをドロップ'}
            </p>
            <p className="drop-port__hint">
              {busy ? statusLabel : 'フォルダまとめてOK · クリックでファイル選択'}
            </p>
      ...
      <section className="ship-card ship-card--inbound anim anim--4">
        <div className="ship-card__banner">
          <span className="ship-card__banner-icon" aria-hidden="true">
            <InboundIcon />
          </span>
          <div className="ship-card__head">
            <div>
              <h2>受取倉庫（社内PC）</h2>
              <p className="ship-card__sub">
                {files.length > 0 ? `${files.length} 件 到着済み` : '到着待ち'}
              </p>
            </div>
          ...
          <div className="parcel-list">
            ...
            {!loading && !error && files.length === 0 && (
              <div className="parcel-empty illus-wrap">
                ...
                <p className="parcel-empty__title">まだファイルは届いていません</p>
                <p className="parcel-empty__desc">自宅PCからアップロードすると、ここに表示されます。</p>
              </div>
            )}
                  ...
                  <li key={f.name} className="parcel-item anim-stagger" style={{ '--i': i } as CSSProperties}>
                    ...
                    <div className="parcel-item__info">
                      <div className="parcel-item__row">
                        ...
                        <span className="tracking-badge">到着済</span>
                      </div>
